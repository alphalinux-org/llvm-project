...
void AlphaTargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  // long double is 128-bit IEEE quad, as GCC signals with this macro.
  ...
  // Match GCC: -mieee defines _IEEE_FP, -mieee-with-inexact also
  // _IEEE_FP_EXACT.
  if (HasIEEE)
    Builder.defineMacro("_IEEE_FP");
  if (HasIEEEInexact)
    Builder.defineMacro("_IEEE_FP_EXACT");
