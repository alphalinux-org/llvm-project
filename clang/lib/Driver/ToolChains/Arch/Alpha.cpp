...
void alpha::getAlphaTargetFeatures(const Driver &D, const ArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mtrap_precision_EQ)) {
    ...
    if (M == "i") {
      ...
      // The su and sui trap modes hand the trap to a software-completion
      // handler, which reads the faulting instruction to work out what to
      // complete -- so it needs the trap to be precise to that instruction.
      // Program precision cannot give it that.  The feature is implied by
      // +ieee regardless, as gcc forces alpha_tp to ALPHA_TP_INSN here; say so
      // rather than let the -mtrap-precision=p look like it took effect.  A
      // 21264 traps precisely in hardware, so there is nothing to warn about.
      ...
      bool PreciseInHardware = CPU == "ev6" || CPU == "ev67" ||
                               CPU == "21264" || CPU == "21264a";
      if (HasSuMode && !PreciseInHardware)
        D.Diag(diag::warn_drv_alpha_fp_completion_needs_trap_precision);
    } else {
      D.Diag(diag::err_drv_unsupported_option_argument)
          << A->getSpelling() << M;
    }
