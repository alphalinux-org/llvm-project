...
ABIArgInfo AlphaABIInfo::classifyArgumentType(QualType Ty, bool IsNamed) const {
  ...
  // An unnamed float or _Complex float goes by reference too, which is what
  // gcc's alpha_pass_by_reference does for SFmode and SCmode.  A 32-bit load
  // into a floating-point register rewrites the bits into the canonical
  // 64-bit form, so the four bytes such an argument would occupy in the
  // register save area are not the value the callee needs to read back.
  // Default argument promotion keeps plain float out of this; _Complex float
  // and _Float32 reach it.
  if (!IsNamed && isSFloating(getContext(), Ty))
    return getNaturalAlignIndirect(Ty, getDataLayout().getAllocaAddrSpace(),
                                   /*ByVal=*/true);
...
RValue AlphaABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  // A _Complex double is passed as its two parts in two consecutive
  // floating-point argument registers, so each part is fetched the way a
  // scalar of the element type would be, bias included.  Reading the pair as
  // one 16-byte object would take it from the integer save area instead.
  // gcc's alpha_gimplify_va_arg_1 recurses on the element type twice for the
  // same reason.  (_Complex float never gets here: it is indirect.)
  if (!IsIndirect)
    if (const auto *CT = Ty->getAs<ComplexType>())
      if (CT->getElementType()->isRealFloatingType()) {
