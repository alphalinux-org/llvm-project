...
ABIArgInfo AlphaABIInfo::classifyArgumentType(QualType Ty, bool IsNamed) const {
  ...
  //
  // An unnamed _Complex float takes the same path here, and this is a known
  // and deliberate divergence from gcc rather than a match.  gcc splits SCmode
  // into two SFmode arguments in alpha_split_complex_arg before
  // alpha_pass_by_reference ever sees it, so gcc passes two pointers in two
  // argument slots where this passes one pointer to the pair in one slot, and
  // every argument after it shifts by a slot.  Two by-reference halves of one
  // argument are not something ABIArgInfo can describe.  gcc's own comment
  // beside alpha_pass_by_reference says "Passing _Complex float as a variable
  // argument never worked on alpha", so nothing depended on either answer.
...
RValue AlphaABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
  ...
  // A _Complex is passed as its two parts in two consecutive argument
  ...
  // element type twice for the same reason.  (_Complex float never gets here:
  // it is indirect.)
  if (!IsIndirect)
    if (const auto *CT = Ty->getAs<ComplexType>()) {
