...
public:
  ...
  // pair -- before the load, and the table itself takes a GOT-adjacent
  // 64KB window that is already the scarce resource on this target.  A chain
  // of compares and branches is usually the better trade here, so leave the
  // tables unbuilt.  (Vectorization has nothing to do with it: these tables
  // are indexed one element at a time.)
