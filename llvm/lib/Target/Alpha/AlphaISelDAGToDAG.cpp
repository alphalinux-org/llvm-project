  ...
  bool SelectInlineAsmMemoryOperand(const SDValue &Op,
    ...
    case InlineAsm::ConstraintCode::m: {
      ...
      if (!SelectADDRri(Op, Base, Offset)) {
        // Fall back to using the address directly as the base with a zero
        // displacement.
        Base = Op;
        Offset = CurDAG->getTargetConstant(0, SDLoc(Op), MVT::i64);
      }
...
SDNode *AlphaDAGToDAGISel::buildConstantInline(int64_t V, const SDLoc &DL) {
  ...
  SDValue High = buildConstant32(static_cast<int32_t>(Hi32), Zero, DL);
