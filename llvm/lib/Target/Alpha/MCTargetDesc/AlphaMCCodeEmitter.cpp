...
void AlphaMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  if (Alpha::hasLituse(MI.getFlags()))
    addLituse(Alpha::getLituse(MI.getFlags()), Fixups);
  // The placeholder that ties a `!literal!N' to its `!lituse_*!N'.  It writes
  // nothing and never reaches the object file; AlphaELFObjectWriter::sortRelocs
  // reads it to put the pair next to each other and then drops it.
  if (Alpha::getSeq(MI.getFlags()))
    Fixups.push_back(MCFixup::create(
        0, MCConstantExpr::create(Alpha::seqMarkValue(MI.getFlags()), Ctx),
        MCFixupKind(Alpha::fixup_alpha_seqmark)));
  uint32_t Bits = getBinaryCodeForInstr(MI, Fixups, STI);
