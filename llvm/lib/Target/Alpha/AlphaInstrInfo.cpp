...
bool AlphaInstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  if (Opc == Alpha::RMW_USTORE) {
    ...
    BuildMI(MBB, MI, DL, get(Alpha::LDA), HiAddr)
        .addImm(Bytes - 1)
        .addReg(Addr);
    BuildMI(MBB, MI, DL, get(Alpha::LDQ_U), Lo).addReg(Addr);
    BuildMI(MBB, MI, DL, get(Alpha::LDQ_U), Hi).addReg(HiAddr);
    ...
    BuildMI(MBB, MI, DL, get(Alpha::STQ_U)).addReg(Hi).addReg(HiAddr);
    ...
    BuildMI(MBB, MI, DL, get(Alpha::STQ_U)).addReg(Lo).addReg(Addr);
