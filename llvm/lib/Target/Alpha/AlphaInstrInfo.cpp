...
// Replace MI with the instructions built ahead of it, First up to End, held
// together in a bundle so nothing can be scheduled between them.
static bool bundleExpansion(MachineBasicBlock &MBB, MachineInstr &MI,
                            const MachineInstrBuilder &First,
...
bool AlphaInstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  if (Opc == Alpha::RMW_USTORE) {
    ...
    return bundleExpansion(MBB, MI, First, End);
  }
