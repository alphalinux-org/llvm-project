...
void AlphaFrameLowering::resetCFIToInitialState(MachineBasicBlock &MBB) const {
  ...
  // $15 is reserved when it is the frame pointer, so it is saved through its
  // own slot rather than by prologue/epilogue insertion and does not appear in
  // getCalleeSavedInfo().  The prologue still gives it an offset rule, so a
  // reset that skipped it would leave that rule standing and send an unwinder
  // to a slot in a frame that no longer exists.
  if (hasFP(MF))
