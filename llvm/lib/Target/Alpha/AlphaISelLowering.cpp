...
SDValue AlphaTargetLowering::LowerF128Convert(SDNode *N,
  ...
  if (Opc == ISD::FP_EXTEND || Opc == ISD::STRICT_FP_EXTEND) {
    ...
    SmallVector<SDValue, 2> Res;
    SDValue Chain = emitOtsCall(
        DAG, DL, Subtarget, InChain, "_OtsConvertFloatTX", {{Alpha::F16, Src}},
        {{Alpha::R16, MVT::i64}, {Alpha::R17, MVT::i64}}, Res);
  ...
  if (Opc == ISD::FP_ROUND || Opc == ISD::STRICT_FP_ROUND) {
    ...
    SmallVector<SDValue, 1> Res;
    Chain = emitOtsCall(DAG, DL, Subtarget, Chain, "_OtsConvertFloatXT",
                        {{Alpha::R16, Lo},
                         {Alpha::R17, Hi},
                         {Alpha::R18, DAG.getConstant(ModeArg, DL, MVT::i64)}},
                        {{Alpha::F0, MVT::f64}}, Res);
    SDValue Result = Res[0];
  ...
  if (Opc == ISD::SINT_TO_FP || Opc == ISD::UINT_TO_FP ||
    ...
    SmallVector<SDValue, 2> Res;
    SDValue Chain =
        emitOtsCall(DAG, DL, Subtarget, InChain, Name, {{Alpha::R16, Src}},
                    {{Alpha::R16, MVT::i64}, {Alpha::R17, MVT::i64}}, Res);
  ...
  if (Opc == ISD::FP_TO_SINT || Opc == ISD::FP_TO_UINT ||
    ...
    SDValue Mode = DAG.getConstant(
        Alpha::getOtsRoundModeArg(Alpha::FPRoundChopped), DL, MVT::i64);

    SmallVector<SDValue, 1> Res;
    Chain =
        emitOtsCall(DAG, DL, Subtarget, Chain, "_OtsCvtXQ",
                    {{Alpha::R16, Lo}, {Alpha::R17, Hi}, {Alpha::R18, Mode}},
                    {{Alpha::R0, MVT::i64}}, Res);
    SDValue Result = Res[0];
...
bool llvm::isAlphaGprelAddressable(const GlobalValue &GV) {
  ...
  // in fact lives in a shared library.  A gp-relative reference is a promise
  // that the definition ends up in this link unit's gp region, and for an
  // external default-visibility declaration the linker cannot keep it: a
  // function resolves to a PLT entry and data to a copy relocation, and
  // R_ALPHA_GPREL{HIGH,LOW} can express neither (`g++ -no-pie` against
  // libstdc++'s typeinfo fails to link on exactly this).  Only a definition
  // here, or an explicit non-default visibility, promises the definition is
  // ours; gcc's local_symbolic_operand asks the same of SYMBOL_REF_LOCAL_P.
  return !GV.isDeclarationForLinker() || !GV.hasDefaultVisibility();
}

// The address of something the linker places in this link unit's gp region:
// ldah/lda over $29 with !gprelhigh/!gprellow.  Forming one is what makes the
// function need a global pointer, so that is recorded here rather than left to
// each caller to remember.
static SDValue getGPRelAddress(SelectionDAG &DAG, const SDLoc &DL,
                               SDValue TargetNode) {
  DAG.getMachineFunction().getInfo<AlphaMachineFunctionInfo>()->setUsesGP();
  SDValue GP = DAG.getRegister(Alpha::R29, MVT::i64);
  SDValue Hi = DAG.getNode(AlphaISD::GPREL_HI, DL, MVT::i64, TargetNode, GP);
  return DAG.getNode(AlphaISD::GPREL_LO, DL, MVT::i64, TargetNode, Hi);
}
...
SDValue AlphaTargetLowering::LowerConstantPool(SDValue Op,
  ...
  return getGPRelAddress(DAG, DL, TCP);
}
...
SDValue AlphaTargetLowering::LowerJumpTable(SDValue Op,
  ...
  return getGPRelAddress(DAG, DL, TJT);
}
...
SDValue AlphaTargetLowering::LowerBlockAddress(SDValue Op,
  ...
  SDValue TBA = DAG.getTargetBlockAddress(BA->getBlockAddress(), MVT::i64,
                                          BA->getOffset());
  return getGPRelAddress(DAG, DL, TBA);
}
...
SDValue AlphaTargetLowering::LowerBR_JT(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Table = getGPRelAddress(DAG, DL, TJT);
...
bool AlphaTargetLowering::isEligibleForTailCallOptimization(
  ...
  for (const ISD::OutputArg &Out : Outs)
    if (Out.Flags.isByVal())
      return false;
  // The tail jump returns straight to our caller, leaving in $29 whatever gp
  // the callee ran on.  Our caller is entitled to find its own gp there: that
  // assumption is exactly what lets the linker rewrite its
  //   ldq $27, f(!literal) ; jsr $26, ($27)(!lituse_jsr) ; ldah/lda !gpdisp
  // into a bsr past f's ldgp and delete the gp reload.  A callee that loads a
  // different gp -- anything reached through the GOT -- therefore corrupts $29
  // for a caller whose reload the linker has already deleted, and the fault
  // surfaces at that caller's next literal load, not here.  An indirect callee
  // cannot be asked, and a runtime-library callee (ExternalSymbol) is the
  // shared libc's or libm's.  gcc refuses both in alpha_function_ok_for_sibcall
  // for the same reason.
  auto *G = dyn_cast<GlobalAddressSDNode>(Callee);
  return G && calleeSharesGP(*G->getGlobal());
}

bool AlphaTargetLowering::supportsTailCallFor(const CallBase *CB) const {
  if (CB->getFunction()->getCallingConv() != CB->getCallingConv())
    return false;
  if (CB->getCallingConv() != CallingConv::C &&
      CB->getCallingConv() != CallingConv::Fast)
    return false;
  if (CB->getFunctionType()->isVarArg())
    return false;

  // Selection also refuses a call that needs outgoing stack space, which is
  // not known here.  Rather than reproduce the argument assignment, accept
  // only arguments that plainly land in the six registers CC_Alpha has: at
  // most six of them, each a scalar no wider than a register.  That covers a
  // symmetric transfer, whose resume function takes one pointer.  A byval
  // argument is a copy in the frame the tail jump tears down, and selection
  // refuses it for that reason, but the type test above has already excluded
  // it as an aggregate.
  if (CB->arg_size() > 6)
    return false;
  ...
  // An indirect callee cannot be asked what global pointer it runs on, so a
  // tail jump to it would leave a foreign gp in $29 for our caller.  This is
  // the case that matters for a symmetric transfer, which resumes through the
  // function pointer held in the coroutine frame.
  const Function *Callee = CB->getCalledFunction();
  return Callee && calleeSharesGP(*Callee);
}

bool AlphaTargetLowering::calleeSharesGP(const GlobalValue &GV) const {
  // A preemptible symbol resolves to a definition in some other DSO, which has
  // its own gp.
  if (!GV.isDSOLocal())
    return false;
  // Under -msmall-text a direct branch reaches the callee with R_ALPHA_BRSGP,
  // which the linker aims past the callee's ldgp: it runs on our gp by
  ...
  // into the same DSO, a gp of its own when it splits the GOT.  This is the
  // rest of decl_has_samegp -- "functions that are not external are defined in
  // this UoT".
  return !GV.isDeclarationForLinker();
}
...
SDValue AlphaTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  bool IsTailCall =
      CLI.IsTailCall && isEligibleForTailCallOptimization(
                            ...
                            CLI.IsVarArg, NumBytes, CLI.Outs, CLI.Callee);
  // musttail is a guarantee, not a hint: the front end has promised the caller
  // that this frame goes away.  Emitting an ordinary call instead would let
  // the stack grow without bound, so say so rather than miscompile silently.
  ...
  // Which of the four forms the call takes.  A direct external call passes the
  // callee symbol as the first operand so the jsr is tagged with the hint and
  // lituse_jsr relocations; a direct local call carries only lituse_jsr; an
  // indirect call has neither; and a bsr names the symbol on its own.  A tail
  // call makes the same choice, and has its own opcode for each answer.
  enum CallForm { BsrForm, HintForm, LocalForm, IndirectForm };
  bool IsDirect = TargetSym.getNode() != nullptr;
  CallForm Form = BsrCall     ? BsrForm
                  : !IsDirect ? IndirectForm
                  : IsLocal   ? LocalForm
                              : HintForm;
  static const unsigned Opcodes[2][4] = {
      {AlphaISD::CALL_BSR, AlphaISD::CALL_DIRECT, AlphaISD::CALL_DIRECT_LOCAL,
       AlphaISD::CALL},
      {AlphaISD::TC_RETURN_BR, AlphaISD::TC_RETURN_DIRECT,
       AlphaISD::TC_RETURN_DIRECT_LOCAL, AlphaISD::TC_RETURN}};
  ...
  if (IsTailCall) {
    if (Glue.getNode())
      Ops.push_back(Glue);
    return DAG.getNode(Opcodes[1][Form], DL, MVT::Other, Ops);
  }
  ...
  Chain = DAG.getNode(Opcodes[0][Form], DL, {MVT::Other, MVT::Glue}, Ops);
  Glue = Chain.getValue(1);
