...
static MachineBasicBlock *emitAtomicRMW(MachineInstr &MI,
  ...
  addNarrowedMemOperands(BuildMI(LoopBB, DL, TII.get(SCOpc), Success)
                             .addReg(New)
                             .addReg(Addr)
                             .addImm(0),
...
static MachineBasicBlock *emitPartialStore(MachineInstr &MI,
  ...
  // Whether the field spans two quadwords depends on the address, so it is a
  // run-time question.  When it does not, AlignedHi is AlignedLo and the second
  // update writes back what it read: correct, but a lock loop that can spin
  // against another processor for nothing.
  Register SameQuad = GPR();
  BuildMI(*BB, MI, DL, TII.get(Alpha::CMPEQ), SameQuad)
      .addReg(AlignedLo)
      .addReg(AlignedHi);
  ...
  MachineBasicBlock *CheckBB = MF.CreateMachineBasicBlock(LLVMBB);
  MachineBasicBlock *LoopHi = MF.CreateMachineBasicBlock(LLVMBB);
  MachineBasicBlock *ExitBB = MF.CreateMachineBasicBlock(LLVMBB);
  MF.insert(It, LoopLo);
  MF.insert(It, CheckBB);
  MF.insert(It, LoopHi);
  MF.insert(It, ExitBB);
  ExitBB->splice(ExitBB->begin(), BB, std::next(MI.getIterator()), BB->end());
  ExitBB->transferSuccessorsAndUpdatePHIs(BB);
  ...
  auto EmitLoop = [&](MachineBasicBlock *Loop, Register Aligned,
    ...
    addNarrowedMemOperands(BuildMI(Loop, DL, TII.get(Alpha::STQ_C), Success)
                               .addReg(Merged)
                               .addReg(Aligned)
                               .addImm(0),
                           MI, MachineMemOperand::MOStore);
