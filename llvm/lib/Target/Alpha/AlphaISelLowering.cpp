...
AlphaTargetLowering::AlphaTargetLowering(const AlphaTargetMachine &TM,
  ...
  for (MVT VT : {MVT::i16, MVT::i32}) {
    ...
    setLoadExtAction(ISD::SEXTLOAD, MVT::i64, VT, Custom);
...
SDValue AlphaTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  return DAG.getNode(ISD::MUL, DL, MVT::i64, Scaled,
...
void AlphaTargetLowering::LowerAsmOperandForConstraint(
  ...
  if (Constraint.size() == 1 &&
      StringRef("IJKLMNOPST").contains(Constraint[0])) {
...
SDValue AlphaTargetLowering::LowerSTORE(SDValue Op, SelectionDAG &DAG) const {
  ...
  // splices the field into them and writes them back.  That has to stay a
  // single instruction until after scheduling: two such stores can fall in one
  // quadword, and if one's reads are hoisted above the other's write-backs the
  // field written first is lost.
...
static SDValue emitOtsCall(SelectionDAG &DAG, const SDLoc &DL,
                           ...
                           SDValue Pv, ArrayRef<SDValue> UseRegs,
                           SDValue Glue) {
...
SDValue AlphaTargetLowering::LowerF128Binary(SDNode *N,
  ...
  Chain = DAG.getCopyToReg(Chain, DL, Alpha::R20,
                           DAG.getConstant(2, DL, MVT::i64), Glue);
  Glue = Chain.getValue(1);
  Chain = DAG.getCopyToReg(Chain, DL, Alpha::R27, Pv, Glue);
  Glue = Chain.getValue(1);

  Chain = emitOtsCall(DAG, DL, Subtarget, Chain, Pv,
...
SDValue AlphaTargetLowering::LowerF128Compare(SDNode *N,
  ...
  case ISD::SETONE:
    Name = "_OtsNeqX";
    break;
  // Unordered comparisons are the logical complement of the ordered opposite.
  // e.g. SETUNE(a,b) = !(a OEQ b) because if either operand is NaN, _OtsEqlX
  // returns 0, so negating gives 1, which is correct for an unordered compare.
  case ISD::SETUNE:
    Name = "_OtsEqlX";
    Negate = true;
    break;
...
SDValue AlphaTargetLowering::LowerF128Bitwise(SDNode *N,
  ...
  case ISD::FABS:
    ...
    break;
  case ISD::FCOPYSIGN: {
    // Copy sign from operand 1; the magnitude stays from operand 0.
    SDValue SignHi =
        splitF128(DAG, DL, MF, DAG.getEntryNode(), N->getOperand(1)).second;
...
bool AlphaTargetLowering::isEligibleForTailCallOptimization(
    CallingConv::ID CalleeCC, bool IsVarArg, unsigned NumStackBytes,
    const SmallVectorImpl<ISD::OutputArg> &Outs) const {
  // The tail-callee returns straight to our caller, so it must use the same
  // (C or fast) convention as this function.
...
SDValue AlphaTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  bool IsTailCall =
      CLI.IsTailCall && isEligibleForTailCallOptimization(
                            CLI.CallConv, CLI.IsVarArg, NumBytes, CLI.Outs);
...
SDValue AlphaTargetLowering::LowerVAARG(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (VT.isFloatingPoint()) {
    SDValue InReg = DAG.getSetCC(
        DL, MVT::i64, Offset, DAG.getConstant(48, DL, MVT::i64), ISD::SETULT);
...
emitAtomicCmpXchg(MachineInstr &MI, MachineBasicBlock *BB, bool Is32 = false) {
  ...
  BuildMI(StoreBB, DL, TII.get(Alpha::BIS), Store)
      ...
      .addReg(New);
  Register Success = MRI.createVirtualRegister(&Alpha::GPRCRegClass);
  BuildMI(StoreBB, DL, TII.get(SCOpc), Success)
      .addReg(Store)
      .addReg(Addr)
      .addImm(0);
