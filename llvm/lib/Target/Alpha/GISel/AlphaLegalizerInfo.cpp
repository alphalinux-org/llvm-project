...
AlphaLegalizerInfo::AlphaLegalizerInfo(const AlphaSubtarget &ST) {
  ...
  getActionDefinitionsBuilder(G_BITCAST)
      .legalIf(all(typeInSet(0, {s32, s64}), typeInSet(1, {s32, s64})))
      .lower();
  ...
  // two, so they go to the SelectionDAG path as well.
  getActionDefinitionsBuilder({G_BLOCK_ADDR, G_CONSTANT_POOL}).unsupported();
