...
static bool definedByFP(Register Reg, const MachineRegisterInfo &MRI,
  ...
  if (Def->isCopy() && Def->getOperand(1).getReg().isPhysical())
    return isFPRegister(Def->getOperand(1).getReg(), MRI, TRI);
  ...
  if (Def->isPHI()) {
    ...
    for (unsigned I = 1, E = Def->getNumOperands(); I < E; I += 2)
      if (Def->getOperand(I).isReg() &&
          definedByFP(Def->getOperand(I).getReg(), MRI, TRI, Seen))
        return true;
  }
  return false;
}
...
static bool usedByFP(Register Reg, const MachineRegisterInfo &MRI,
  ...
  for (const MachineInstr &Use : MRI.use_nodbg_instructions(Reg)) {
    ...
    if (Use.isCopy() && Use.getOperand(0).getReg().isPhysical() &&
        isFPRegister(Use.getOperand(0).getReg(), MRI, TRI))
      return true;
  }
  return false;
}

AlphaRegisterBankInfo::AlphaRegisterBankInfo(const TargetRegisterInfo &TRI)
    : AlphaGenRegisterBankInfo() {}
...
AlphaRegisterBankInfo::getInstrMapping(const MachineInstr &MI) const {
  ...
  case G_STORE: {
    ...
    bool IsFP = Opc == G_LOAD ? usedByFP(ValReg, MRI, TRI)
                              : definedByFP(ValReg, MRI, TRI);
  ...
  case G_PHI: {
    ...
    const TargetRegisterInfo &TRI = *MF.getSubtarget().getRegisterInfo();
    bool IsFP = usedByFP(MI.getOperand(0).getReg(), MRI, TRI);
    for (unsigned I = 1, E = NumOperands; I < E && !IsFP; I += 2)
      IsFP = definedByFP(MI.getOperand(I).getReg(), MRI, TRI);
    OperandsMapping =
        &Alpha::ValueMappings[IsFP ? Alpha::FPR3OpsIdx : Alpha::GPR3OpsIdx];
    break;
  }
  ...
  case G_BITCAST: {
    const TargetRegisterInfo &TRI = *MF.getSubtarget().getRegisterInfo();
    bool DstFP = usedByFP(MI.getOperand(0).getReg(), MRI, TRI);
    bool SrcFP = definedByFP(MI.getOperand(1).getReg(), MRI, TRI);
    OperandsMapping = getOperandsMapping(
  ...
  case G_CONSTANT_FOLD_BARRIER: {
    ...
    if (!IsFP && NumOperands > 1 && MI.getOperand(1).isReg())
      IsFP = definedByFP(MI.getOperand(1).getReg(), MRI, TRI);
    OperandsMapping =
        &Alpha::ValueMappings[IsFP ? Alpha::FPR3OpsIdx : Alpha::GPR3OpsIdx];
    if (NumOperands > 1)
      OperandsMapping = getOperandsMapping(
  ...
  // A result register and an index or symbol that is not one.
  case G_JUMP_TABLE:
  case G_BLOCK_ADDR:
  case G_CONSTANT_POOL:
    OperandsMapping =
        getOperandsMapping({&Alpha::ValueMappings[Alpha::GPR3OpsIdx], nullptr});
    break;
  // A pointer register and a jump table index.
  case G_BRJT:
    OperandsMapping =
        getOperandsMapping({&Alpha::ValueMappings[Alpha::GPR3OpsIdx], nullptr,
                            &Alpha::ValueMappings[Alpha::GPR3OpsIdx]});
  ...
  case G_ATOMICRMW_NAND:
  case G_ATOMICRMW_OR:
  case G_ATOMICRMW_XOR:
  case G_ATOMICRMW_MAX:
  case G_ATOMICRMW_MIN:
  case G_ATOMICRMW_UMAX:
  case G_ATOMICRMW_UMIN:
