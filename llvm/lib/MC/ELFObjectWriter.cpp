...
void ELFWriter::writeSymbol(SymbolTableWriter &Writer, uint32_t StringIndex,
  ...
  // A weak alias such as `.weak foo; foo = bar` carries no st_other bits of its
  // own, so inherit them from bar -- but only the bits the target says describe
  // a procedure rather than something an alias must not acquire on its own.
...
void ELFObjectWriter::executePostLayoutBinding() {
  ...
  for (const Symver &S : Symvers) {
    ...
    if (!Other)
      Other = resolveAliasedOther(
          Symbol, getContext().getAsmInfo().getInheritedSTOtherMask());
