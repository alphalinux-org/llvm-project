...
; reads of one must not be hoisted above the write-backs of the other.  Being
; one instruction until now is not enough on its own: this expansion runs
; before the post-RA scheduler, not after it, so the pieces go into a bundle.
