...
; -mieee implies -mtrap-precision=i: software completion needs the trap to be
; precise to the instruction it has to complete.  gcc forces alpha_tp to
; ALPHA_TP_INSN for TARGET_IEEE for the same reason.
; RUN: llc -mtriple=alpha-unknown-linux-gnu -mcpu=ev5 -mattr=+ieee < %s | FileCheck %s
; RUN: llc -mtriple=alpha-unknown-linux-gnu -mcpu=ev5 -mattr=+ieee-with-inexact < %s | FileCheck %s --check-prefix=SUI
