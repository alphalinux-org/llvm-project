...
; The amounts are the point, and they were all wildcarded.  The frame is
; 5000 * 8 = 40000 bytes, so the prologue subtracts it as 25536 - 65536 and the
; epilogue adds it back as 65536 - 25536.
; CHECK-LABEL: big:
...
; the same two-instruction build, and that is the part no check reached: the
; last element sits at 4999 * 8 = 39992, written as 65536 - 25544.  An offset
; wrong by any amount would have passed.
; CHECK:      ldah $0, 1($31)
