...
; RUN: llc -mtriple=alpha-unknown-linux-gnu -global-isel -o - %s \
...
; An outgoing argument's stack slot is 8-byte aligned by construction: the
; argument area is a run of 8-byte slots at multiples of eight from $sp.  The
; pointer info is a plain Stack pseudo-source-value with no frame index behind
; it, so nothing can infer that alignment from it -- getting it wrong turns
; every stack-passed argument into an unaligned store sequence.
; CHECK-LABEL: name: manycall
; CHECK: G_STORE %{{[0-9]+}}(s64), %{{[0-9]+}}(p0) :: (store (s64) into stack)
; CHECK: G_STORE %{{[0-9]+}}(s64), %{{[0-9]+}}(p0) :: (store (s64) into stack + 8)
; ASM-LABEL: manycall:
; ASM: stq ${{[0-9]+}}, 0($30)
; ASM: stq ${{[0-9]+}}, 8($30)
; ASM-NOT: stq_u
define void @manycall() {
  call void @many(i64 1, i64 2, i64 3, i64 4, i64 5, i64 6, i64 7, i64 8)
  ret void
}
