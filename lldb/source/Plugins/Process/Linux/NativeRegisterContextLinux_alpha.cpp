...
NativeRegisterContextLinux_alpha::ReadRegister(const RegisterInfo *reg_info,
  ...
  // The floating registers hold IEEE754 values, which SetUInt64 would tag as
  // an unsigned integer and print as one.  SetFromMemoryData reads the
  // encoding out of the RegisterInfo, so each register comes back as what it
  // is.
  Status convert_error;
  reg_value.SetFromMemoryData(*reg_info, &value, reg_info->byte_size,
                              endian::InlHostByteOrder(), convert_error);
...
Status NativeRegisterContextLinux_alpha::WriteRegister(
  ...
  // GetAsUInt64 converts a floating-point RegisterValue numerically, so
  // writing 0.5 to $f0 would poke 0.  Take the bytes the register actually
  // holds instead, the way ReadRegister builds them.
  uint64_t value = 0;
  Status convert_error;
  reg_value.GetAsMemoryData(*reg_info, &value, sizeof(value),
                            endian::InlHostByteOrder(), convert_error);
  if (convert_error.Fail())
    return convert_error;
...
Status NativeRegisterContextLinux_alpha::ReadAllRegisterValues(
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (error.Fail())
      return error;
    // Save the register's bits, not a numeric conversion of them: the
    // save/restore path around an expression evaluation has to put $f0-$f30
    // back exactly as it found them.
    Status convert_error;
    value.GetAsMemoryData(*reg_info, &dst[i], sizeof(uint64_t),
                          endian::InlHostByteOrder(), convert_error);
    if (convert_error.Fail())
      return convert_error;
  }
...
Status NativeRegisterContextLinux_alpha::WriteAllRegisterValues(
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    RegisterValue value;
    Status convert_error;
    value.SetFromMemoryData(*reg_info, &src[i], sizeof(uint64_t),
                            endian::InlHostByteOrder(), convert_error);
    if (convert_error.Fail())
      return convert_error;
    Status error = WriteRegister(reg_info, value);
    if (error.Fail())
      return error;
  }
  return Status();
