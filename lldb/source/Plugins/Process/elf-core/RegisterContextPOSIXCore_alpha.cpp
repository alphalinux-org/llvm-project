...
bool RegisterContextCorePOSIX_alpha::ReadRegister(const RegisterInfo *reg_info,
  ...
  if (IsFPR(reg)) {
    ...
    if (offset == start + reg_info->byte_size) {
      // The floating registers hold IEEE754 values, which SetUInt would tag as
      // an unsigned integer and print as one.  SetFromMemoryData reads the
      // encoding out of the RegisterInfo, so each register comes back as what
      // it is.
      Status error;
      value.SetFromMemoryData(*reg_info, &v, reg_info->byte_size,
