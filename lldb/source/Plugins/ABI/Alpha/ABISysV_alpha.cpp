...
Status ABISysV_alpha::SetReturnValueObject(StackFrameSP &frame_sp,
  ...
  if (type.IsIntegerOrEnumerationType(is_signed) || type.IsPointerType()) {
    ...
    if (value.SetValueFromData(*f0, data, 0, true).Fail() ||
        !reg_ctx->WriteRegister(f0, value))
...
ABISysV_alpha::GetReturnValueObjectImpl(Thread &thread,
  ...
  // callee a buffer in $16, and the callee returns that pointer in $0.
...
UnwindPlanSP ABISysV_alpha::CreateDefaultUnwindPlan() {
  // A frame keeps no chain pointer, so without unwind information there is
  // nothing to walk: the best guess is the state a frame is in before its
  // prologue has run, which is right for a leaf frame and for the frame a
  // signal interrupted.  The backend always emits .eh_frame, so this is only
  // reached for a frame whose module has no unwind information at all.
  UnwindPlan::Row row;
  row.GetCFAValue().SetIsRegisterPlusOffset(gpr_r30_alpha, 0);
  row.SetRegisterLocationToRegister(gpr_pc_alpha, gpr_r26_alpha, true);
...
bool ABISysV_alpha::RegisterIsCalleeSaved(const RegisterInfo *reg_info) {
  ...
  // $9-$15 and $f2-$f9 are preserved across a call, as are the stack and global
  // pointers.  $26 holds the return address the frame needs to unwind, so it is
  // treated as preserved here too.
